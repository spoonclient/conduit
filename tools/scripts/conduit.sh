...
startStop=$var1
configFile=$var2
envfile=$CONDUIT_DIR/conf/conduit.env
...
#set classpath
export CLASSPATH=`ls $CONDUIT_DIR/lib/*jar | tr "\n" :`;
export CLASSPATH=$CONDUIT_DIR/conf:$CLASSPATH:$HADOOP_CONF_DIR
...
case $startStop in
  ...
  (start)
   ...
   nohup java -cp "$CLASSPATH" com.inmobi.conduit.Conduit $configFile 2>&1 &
   if [ $? -eq 0 ]
    then
  ...
  (stop)
    ...
    if [ -f $_CONDUIT_DAEMON_PIDFILE ]; then
      if kill -0 `cat $_CONDUIT_DAEMON_PIDFILE` > /dev/null 2>&1; then
        ...
        kill -s SIGINT `cat $_CONDUIT_DAEMON_PIDFILE`
      while :
        do 
          if kill -0 `cat $_CONDUIT_DAEMON_PIDFILE` > /dev/null 2>&1; then
             echo -n "."
