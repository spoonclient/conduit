...
run_conduit() {
  ...
  set -x
  exec $JAVA_HOME/bin/java $JAVA_OPTS -cp "$CONDUIT_CLASSPATH" \
...
if [ -n "${CONDUIT_CLASSPATH}" ] ; then
  CONDUIT_CLASSPATH="${CONDUIT_HOME}/lib/*:$DEV_CLASSPATH:$CONDUIT_CLASSPATH"
else
  CONDUIT_CLASSPATH="${CONDUIT_HOME}/lib/*:$DEV_CLASSPATH"
fi
